...
const Inicio: React.FC = () => {
  ...
  const geradorColor = (data) => {
    return data.map((value) => (value < 0 ? "rgba(255, 99, 132, 0.6)" : "rgba(75, 192, 192, 0.6)"));
  };
    ...
    datasets: [
      {
        ...
        borderColor: geradorColor(valoresLiquidos).map(
          (color) => color.replace("0.6", "1") // Aumenta a opacidade para a borda
        ),
        borderWidth: 1,
        tension: 0.1
      }
  ...
  return (
    <>
      <h2 className="text-center text-2xl font-bold mb-4 hidden lg:block">Resumo rapido </h2>
